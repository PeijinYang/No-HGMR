...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        # erase all overlap
        # self.R_pv = self.R_pv - self.R_cart
        # self.R_pv[self.R_pv < 0] = 0
        #
        # self.R_cart = self.R_cart - self.R
        # self.R_cart[self.R_cart < 0] = 0

        # erase overlap between purchase and add-to-cart
        # self.R_cart = self.R_cart - self.R
        # self.R_cart[self.R_cart < 0] = 0

        # erase overlap between page-view and add-to-cart
        # self.R_pv = self.R_pv - self.R_cart
        # self.R_pv[self.R_pv < 0] = 0

        for uid in range(self.R_pv.shape[0]):
            self.pv_set[uid] = self.R_pv[uid].nonzero()[1].tolist()
            self.cart_set[uid] = self.R_cart[uid].nonzero()[1].tolist()
