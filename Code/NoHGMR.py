...
class GHCF(object):
    def __init__(self, max_item_view, max_item_cart, max_item_buy,data_config):
        ...
        *********************************************************
        ...
        # self.input_u = tf.keras.Input([None, 1], name="input_uid", dtype=tf.int32)
        # self.lable_view = tf.keras.Input([None, self.max_item_view], name="lable_view", dtype=tf.int32)
        # self.lable_cart = tf.keras.Input(dtype=tf.int32, shape=[None, self.max_item_cart], name="lable_cart")
        # self.lable_buy = tf.keras.Input(dtype=tf.int32, shape=[None, self.max_item_buy], name="lable_buy")
        #
        # self.users = tf.keras.Input(dtype=tf.int32, shape=(None,))
        # self.pos_items = tf.keras.Input(dtype=tf.int32, shape=(None,))
        #
        # self.node_dropout_flag = args.node_dropout_flag
        # self.node_dropout = tf.keras.Input(dtype=tf.float32, shape=[None])
        # self.mess_dropout = tf.keras.Input(dtype=tf.float32, shape=[None])
        ...
        *********************************************************
        ...
        # self.batch_ratings = tf.einsum('ajk->aj', self.dot)
        ...
        # self.pos_rv = tf.einsum('abc->ab', self.pos_rv)
        ...
        # self.pos_rc = tf.einsum('abc->ab', self.pos_rc)
        ...
        # self.pos_rb = tf.einsum('abc->ab', self.pos_rb)
        ...
        self.mf_loss, self.emb_loss, self.independent_loss = self.create_non_sampling_loss()
        self.loss = self.mf_loss + self.emb_loss + self.independent_loss
    ...
    def _init_weights(self):
        ...
        for k in range(self.n_layers):
            ...
            all_weights['W_K_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k] // self.n_K, 1]), name='W_gc_%d' % k)


            all_weights['W_attention_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k] // self.n_K, self.weight_size_list[k] // self.n_K]), name='W_gc_%d' % k)

            all_weights['a_attention_%d' % k] = tf.Variable(
                initializer([2 * self.weight_size_list[k] // self.n_K, 1]), name='W_gc_%d' % k)

            all_weights['W2_gc_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]]), name='W_gc_%d' % k)

            all_weights['W3_gc_%d' % k] = tf.Variable(
                initializer([self.weight_size_list[k], self.weight_size_list[k + 1]]), name='W_gc_%d' % k)
        ...
        for l in range(self.n_K):
            all_weights['W_intent_%d' % l] = tf.Variable(
                initializer([self.emb_dim, self.emb_dim]), name='W_intent_%d' % l)

            all_weights['E_intent_%d' % l] = tf.Variable(
                initializer([self.n_relations, self.emb_dim]), name='E_intent_%d' % l)


        all_weights['W_loss'] = tf.Variable(
            initializer([1, self.n_relations]), name='W_imp')

        all_weights['W_beh'] = tf.Variable(
            initializer([1, self.n_relations]), name='W_imp')
    ...
    def _create_gcn_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            embeddings_pv_2 = tf.multiply(embeddings_pv, r0)
            ...
            embeddings_cart_2 = tf.multiply(embeddings_cart, r1)

            temp_embed = []
            for f in range(self.n_fold):
                temp_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat_buy[f], embeddings))
            embeddings_buy = tf.concat(temp_embed, 0)
            embeddings_buy_2 = tf.multiply(embeddings_buy, r2)

            # correlation embeddings
            relationship_pv_cart = tf.multiply(r0, r1)
            relationship_pv_buy = tf.multiply(r0, r2)
            relationship_cart_buy = tf.multiply(r1, r2)

            homogeneity_pv_cart = 1 * tf.multiply(tf.multiply(embeddings_pv, relationship_pv_cart), embeddings_cart)
            homogeneity_pv_buy = 1 * tf.multiply(tf.multiply(embeddings_pv, relationship_pv_buy), embeddings_buy)
            homogeneity_cart_buy = 1 * tf.multiply(tf.multiply(embeddings_cart, relationship_cart_buy), embeddings_buy)


            #
            embeddings = self.coefficient[0]* (embeddings_pv_2 + homogeneity_pv_cart + homogeneity_pv_buy) + \
                         self.coefficient[1]* (embeddings_cart_2 + homogeneity_pv_cart + homogeneity_cart_buy) + \
                         self.coefficient[2] * (embeddings_buy_2 + homogeneity_pv_buy + homogeneity_cart_buy)
    ...
    def create_non_sampling_loss(self):
        ...
        # temp = tf.matmul(self.ia_embeddings, self.uid, transpose_b=True)

        # loss_importance = tf.nn.relu(self.weights['W_loss'])
        ...
        all_embeddings = tf.concat([self.ua_embeddings, self.ia_embeddings], 0)

        all_embeddings_K = tf.split(all_embeddings, self.n_K, 1)

        independent_loss = 0
        for i in range(self.n_K - 1):
            for j in range(i+1, self.n_K):
                x = all_embeddings_K[i]
                y = all_embeddings_K[j]
                independent_loss += self.cosine_similarity(x, y)
        independent_loss = independent_loss * 0.001

        return loss, emb_loss, independent_loss

    def cosine_similarity(self, X1, X2):
        norm_x = tf.nn.l2_normalize(X1, 1)
        norm_y = tf.nn.l2_normalize(X2, 1)
        dcor = tf.reduce_sum(tf.reduce_sum(tf.multiply(norm_x, norm_y), 1), 0)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            _, batch_loss, batch_mf_loss, batch_emb_loss, batch_independent_loss = sess.run(
                [model.opt, model.loss, model.mf_loss, model.emb_loss, model.independent_loss],
            ...
            independent_loss = batch_independent_loss / n_batch
        ...
        if (epoch + 1) % 10 != 0:
            if args.verbose > 0 and epoch % args.verbose == 0:
                perf_str = 'Epoch %d [%.1fs]: train==[%.5f=%.5f + %.5f + %.5f]' % (
                    epoch, time() - t1, loss, mf_loss, emb_loss, independent_loss)
                print(perf_str)
            continue
